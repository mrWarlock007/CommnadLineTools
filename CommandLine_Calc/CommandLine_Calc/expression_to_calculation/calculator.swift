...
private extension Expression {
    ...
    func evaluate() throws -> Number {
        ...
        case .module(let lhs, let rhs): return try lhs.evaluate() % rhs.evaluate()
        }
    }
...
private extension Number {
    ...
    static func %(lhs: Number, rhs: Number) -> Number {
        switch (lhs, rhs) {
        case let (.int(i),    .int(ii)):    return .int(i % ii)
        case let (.int(i),    .double(d)):  return .double(Double(i).truncatingRemainder(dividingBy: d))
        case let (.double(d), .int(i)):     return .double(d.truncatingRemainder(dividingBy: Double(i)))
        case let (.double(d), .double(dd)): return .double(d.truncatingRemainder(dividingBy: dd))
        }
    }
